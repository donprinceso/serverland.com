...
function GetBlogPost($id=null,$Tagsid=null){
    global $con;
    $con = new mysqli('localhost', "root", "Changeless11!")or die("Sorry can`t conncet to the database");
    ...
    elseif (!empty ($Tagsid))
    {
      $query = $con->query("SELECT blog_posts.* FROM blog_post_tags LEFT JOIN (blog_posts) ON (blog_post_tags.postID = blog_posts.id) WHERE blog_post_tags.tagID =" . $tag_id . " ORDER BY blog_posts.id DESC");
    }
    else
    {
